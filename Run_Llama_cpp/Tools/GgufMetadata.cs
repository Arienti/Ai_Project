...
namespace Run_Llama_cpp.Tools
{
    public class GgufMetadata
    {
        ...
        public static GgufMetadata ReadFromFile(string modelPath)
        {
            ...
            // Fill NLayers, Dim, and FType from filename if missing
        ...
        {
            // Mapping table for known models (B -> NLayers, Dim)
            var table = new Dictionary<double, (uint NLayers, uint Dim)>()
            {
                ...
                { 13, (40, 5120) },
                { 30, (60, 8192) },
                { 33, (64, 8192) },  // example: LLaMA-33B
                { 65, (80, 12288) },
                { 70, (80, 12288) }, // example: Mistral-70B
                { 120, (96, 16384) },
                { 150, (96, 16384) }, 
                // You can continue adding all models up to 175B, 200B+, etc.
            };

            // Find nearest size <= sizeB
            double nearest = 3;
        ...
        {
            return $"ModelName: {ModelName}, NCtx: {NCtx}, NLayers: {NLayers}, Dim: {Dim}, FType: {FType}";
        }
    }
}
